...
var arr$, code_line, content_line, count, gen_table, log, nest_in, nest_table, num$, obj$, parse, str$, tokenize;
...
nest_in = function(list, item, level) {
  if (item.indent === 0) {
    delete item.indent;
    ...
    item.indent -= 1;
    return nest_in(list.last, item, level + 1);
  }
};
...
nest_table = function(table) {
  ...
  table.forEach(function(line) {
    return nest_in(ret, line, 0);
  });
  return ret;
};

code_line = function(list) {
  return list.length > 0;
};

content_line = function(line) {
  return line.trim().length > 0;
};

exports.parse = parse = function(content) {
  var lines, res, ret, table;
  lines = content.split('\n');
